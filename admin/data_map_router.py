...
def migrate_all():
    ...
    if caja_dir.exists():
        for caja_file in sorted(caja_dir.glob("*.json")):
            try:
                data = json.loads(caja_file.read_text(encoding="utf-8"))
                ...
                results["errores"].append(f"caja file {caja_file.name}: {e}")

    # Pagos
    pagos_dir = DATA_DIR / "pagos"
    if pagos_dir.exists():
        for pagos_file in sorted(pagos_dir.glob("*.json")):
            try:
                ...
                results["errores"].append(f"pagos file {pagos_file.name}: {e}")
