...
def set_day_blocks(
    *,
    ...
    # Solo inicializa slotMinutes si no existe — nunca sobreescribe
    if "slotMinutes" not in schedule:
        schedule["slotMinutes"] = slot_minutes
    if "slotMinutesTelemed" not in schedule:
        schedule["slotMinutesTelemed"] = slot_minutes
